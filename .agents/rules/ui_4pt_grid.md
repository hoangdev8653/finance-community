...
# UI/UX Pro: Image-to-Code & Production UX Standard

When coding UI from mockup images (e.g., generated by GPT, Midjourney, Figma screenshots, or design references), you MUST follow this protocol to translate visual mockups into production-grade, interactive code.

---

## 21. Image-to-Code Translation Protocol

### The Core Principle
Mockup images depict a **single, static, idealized moment**. Code must be **dynamic, accessible, robust, and maintainable**. 
Never blindly copy arbitrary pixel positions or random visual artifacts from an image. Translate the visual intent into clean design-system components.

### Step 1: Layout & Spatial Quantization
- **Map to 4px Grid**: Convert visual spacing in the image to the nearest standard Tailwind spacing scale (`p-3`, `p-4`, `p-6`, `gap-4`, `gap-6`, `space-y-4`). Never emit arbitrary values like `p-[19px]` or `gap-[13px]`.
- **Structural Layout Hierarchy**: Break down the image into standard primitives:
  1. Root Page Container (`min-h-screen bg-background text-foreground p-4 md:p-6 lg:p-8`)
  2. Main Sections / Grids (`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6` or `flex flex-col lg:flex-row gap-6`)
  3. Cards / Panels (`bg-card text-card-foreground border border-border rounded-xl p-5 shadow-sm`)
  4. Header / Action Bars (`flex items-center justify-between pb-4 border-b border-border`)

### Step 2: Semantic Color Mapping
- Do NOT hardcode arbitrary HEX colors spotted in the image (`#1a1f2c`, `#334155`).
- Translate image colors into project semantic tokens:
  - Surface backgrounds: `bg-background`, `bg-card`, `bg-muted/50`, `bg-secondary`
  - Text: `text-foreground` (titles), `text-muted-foreground` (subtitles/descriptions/meta)
  - Borders & Dividers: `border-border` or `border-border/60`
  - Accent / Primary actions: `bg-primary text-primary-foreground hover:bg-primary/90`
- **Dark & Light Mode Parity**: Every color mapped from the image must look intentional in both dark and light modes via CSS variable tokens.

### Step 3: Icon & Asset Reconstruction
- Replace visual icons in the image with standard **Lucide React** icons (`lucide-react`).
- Maintain consistent icon sizing: `w-4 h-4` (inline/small), `w-5 h-5` (standard buttons/navigation), `w-6 h-6` (feature/card headers).
- Always use `aria-hidden="true"` or wrap icon buttons with descriptive `aria-label`.

---

## 22. Interactive State Completeness (Beyond the Static Image)

Mockups never show edge cases. Every component generated from an image MUST implement the **5 Essential UX States**:

1. **Default State**:
   - Polished rendering aligned with the mockup's visual hierarchy.
2. **Hover & Active States (Micro-interactions)**:
   - Interactive cards: `transition-all duration-200 hover:border-primary/50 hover:shadow-md hover:-translate-y-0.5`
   - Buttons: `transition-all duration-150 active:scale-[0.98]`
   - List rows: `hover:bg-muted/50 transition-colors`
...
3. **Loading State (Skeleton UI)**:
   - Never leave pages blank or with raw spinners during async fetch.
   - Provide shimmering skeleton placeholders that match the exact shape of cards, tables, and metric badges (`animate-pulse bg-muted rounded`).
4. **Empty State**:
   - When lists, tables, or search results have 0 items:
     - Prominent, subtle icon in a rounded container (`p-3 bg-muted/50 rounded-full text-muted-foreground`)
     - Clear headline (e.g., "Chưa có dữ liệu")
     - Helpful explanatory text (e.g., "Các mục mới sẽ hiển thị tại đây khi được thêm.")
     - Primary action CTA button (e.g., "Thêm mới")
5. **Error & Validation States**:
   - Clear visual feedback with destructive tokens (`text-destructive`, `border-destructive/50`, `bg-destructive/10`).
   - Accessible error text beneath input fields.

---

## 23. Responsive Adaptation for Desktop-Centric Mockups

Mockup images are typically 1440px desktop layouts. When implementing:

- **Mobile First / Progressive Enhancement**:
  - Grid: `grid-cols-1 md:grid-cols-2 lg:grid-cols-3`
  - Sidebars: Collapsible drawer/sheet on mobile (`< md`), sticky sidebar on desktop (`>= lg`).
  - Tables: Responsive wrapper (`overflow-x-auto`) or mobile card view fallback.
  - Action Bars: Wrap flex items cleanly on small screens (`flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4`).
- **Touch Target Accessibility**:
  - Minimum touch target for interactive elements: `min-h-[40px]` or `min-h-[44px]` on mobile.

---

## 24. Finance & Community Aesthetic Guidelines

For finance, trading, and community platforms:

- **Data Density & Readability**:
  - High information density without visual clutter. Use subtle dividers (`divide-y divide-border/60`).
  - Metric numbers and financial figures should use `font-mono` or `tabular-nums` to prevent layout shift during updates.
- **Financial Status Indicators**:
  - Positive / Profit: `text-emerald-500 bg-emerald-500/10 border-emerald-500/20`
  - Negative / Loss: `text-rose-500 bg-rose-500/10 border-rose-500/20`
  - Neutral / Pending: `text-slate-500 bg-slate-500/10 border-slate-500/20`
  - Warning / Alert: `text-amber-500 bg-amber-500/10 border-amber-500/20`
- **Cards & Surfaces**:
  - Crisp, subtle borders over loud shadows: `border border-border/80 bg-card/95 backdrop-blur-sm`.
