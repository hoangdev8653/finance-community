...
export function LearningQuiz({ postId }: LearningQuizProps) {
  ...
  async function submit() {
    const next = await learningService.submitQuiz(postId, Object.entries(answers).map(([questionId, optionId]) => ({ questionId, optionId })));
    setResult(next); setSubmitted(true);
  }
  ...
  return <section className="mt-10 rounded-2xl border border-border bg-muted/20 p-5 sm:p-7" aria-labelledby="learning-quiz-title">
    ...
    <div className="mt-7 flex flex-wrap items-center gap-3"><button type="button" disabled={!allAnswered || submitted} onClick={() => void submit()} className="min-h-11 rounded-xl bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">Kiểm tra đáp án</button>{result && <span className="text-sm font-semibold">Kết quả: {result.score}/{result.total} ({result.percentage}%)</span>}{submitted && <button type="button" onClick={() => { setAnswers({}); setSubmitted(false); setResult(null); }} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-semibold"><RotateCcw className="h-4 w-4" />Làm lại</button>}</div>
    <p className="mt-4 text-sm text-muted-foreground">Quiz giúp tự kiểm tra kiến thức và không ảnh hưởng đến việc mở khóa bài học tiếp theo.</p>
  </section>;
}
