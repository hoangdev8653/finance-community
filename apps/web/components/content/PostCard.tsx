    ...
    <article className="group flex flex-col sm:flex-row items-start gap-4 sm:gap-6 py-5 border-b border-dashed border-border last:border-b-0 transition-colors">
      ...
      <div className="flex-1 min-w-0 space-y-2 pt-1 w-full">
        ...
        <h2 className="line-clamp-2">
          <Link
            href={postHref}
            className="font-heading text-lg sm:text-xl lg:text-2xl font-bold text-foreground group-hover:text-primary transition-colors leading-snug"
          >
            {post.title}
          </Link>
        </h2>
        ...
        <div className="flex items-center justify-between gap-2 text-sm font-semibold">
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <span className="text-primary font-bold hover:underline cursor-pointer truncate">
              {displayCategory}
            </span>
            <span className="text-muted-foreground/60 font-normal">-</span>
            <time dateTime={displayDate} suppressHydrationWarning className="text-muted-foreground font-medium text-xs sm:text-sm shrink-0">
              {timeLabel}
            </time>
          </div>
        ...
        {post.metaDescription && (
          <p className="text-sm sm:text-base text-muted-foreground line-clamp-3 leading-relaxed font-normal pt-1">
            {post.metaDescription}
          </p>
        )}
      </div>
