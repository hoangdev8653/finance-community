...
export function MobileNavigation() {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 block lg:hidden border-t border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md"
    >
        ...
        {mobileNavConfig.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'relative flex flex-col items-center justify-center gap-1 py-1.5 px-3 rounded-xl transition-all duration-150',
                isActive
            ...
            >
              ...
              <span className="text-[11px] font-medium tracking-tight">{t(item.labelKey)}</span>
              {isActive && (
                <span className="absolute -bottom-0.5 h-1 w-5 rounded-full bg-blue-600 dark:bg-blue-400" />
              )}
            </Link>
          );
        })}
