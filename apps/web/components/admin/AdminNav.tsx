...
export function AdminNav() {
  ...
  return (
    <aside className="group/sidebar fixed left-0 top-14 z-30 hidden h-[calc(100vh-3.5rem)] w-[248px] shrink-0 flex-col overflow-hidden border-r border-border bg-background/95 px-4 py-4 lg:flex">
      <nav aria-label="Điều hướng quản trị" className="flex-1 space-y-4 overflow-y-auto pr-1">
        {(['Không gian làm việc', 'Nội dung', 'Cộng đồng', 'Phân loại nội dung', 'Quản trị'] as const).map(
          (group) => (
              ...
              {ADMIN_LINKS.filter((link) => link.group === group).map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`relative flex min-h-10 items-center gap-3 rounded-lg px-3 text-sm font-medium transition-colors ${
                      active
      ...
      <div className="mt-auto rounded-xl border border-border bg-surface p-3 shadow-2xs">
        <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600">
          <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
          <span>Hệ thống đang hoạt động</span>
        </div>
        <p className="mt-1 pl-4 text-xs text-muted-foreground">Kiểm tra lần cuối: vừa xong</p>
      </div>
    </aside>
  );
}
