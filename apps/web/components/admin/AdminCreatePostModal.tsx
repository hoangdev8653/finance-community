...
export function AdminCreatePostModal({ open, onClose, onCreated }: Props) {
  ...
  const [title, setTitle] = useState(''); const [contentType, setContentType] = useState<'SERIES'|'COMMUNITY'|'NEWS'>('SERIES'); const [categoryId, setCategoryId] = useState<string>(); const [tags, setTags] = useState<string[]>([]); const [coverMediaId, setCoverMediaId] = useState<string|null>(null); const [body, setBody] = useState(''); const [metaTitle, setMetaTitle] = useState(''); const [metaDescription, setMetaDescription] = useState('');
  if (!open) return null;
