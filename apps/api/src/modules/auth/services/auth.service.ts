...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    // 1. Check if email already registered in Database or memory fallback
    if (this.usersRepo) {
      try {
    ...
    // 2. Check if username is already taken in Database or memory fallback
    ...
    // Keep memory fallback in sync for testing or DB-less mode
